...
const EditProduct = () => {
      ...
      const loadData = async () => {
        try {
          ...
          const [categoriesResponse, productResponse] = await Promise.all([
            axiosInstance.get('/api/admin/categories').catch(error => {
              console.error('Kategori yükleme hatası:', error);
              return { data: [] }; // Hata durumunda boş dizi döndür
            }),
          ...
          setFormData({
            ...
            isDiscount: Boolean(productData.isDiscount),
            discountPrice: productData.discountPrice || '',
            discountEndDate: productData.discountEndDate ? new Date(productData.discountEndDate).toISOString().slice(0, 16) : '',
            isFlash: Boolean(productData.isFlash),
            flashEndDate: productData.flashEndDate ? new Date(productData.flashEndDate).toISOString().slice(0, 16) : '',
            flashDiscountRate: productData.flashDiscountRate || '',
          });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axiosInstance.put(`/api/admin/products/${id}`, submitData);
